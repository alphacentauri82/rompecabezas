/* Variables para funciones adicionales: Contador y Limitador de Movimientos, 
Adicion al mezclador de piezas */
var TimesMixPiezas = 60;
var CantMaxMovimientos = 80;
var movRealizados = 0;
var movRestantes = 0;

// Arreglo que contiene las intrucciones del juego 
var instrucciones = [
  "Mueve las piezas usando las flechas del teclado: ← | ↑ | → | ↓",
  "Ganas si lo resuelves en " + CantMaxMovimientos + " movimientos."
];
...
// Grilla Ganadora - guardo aqui para chequear mas adelante
var grillaGanadora = [
    [1, 2, 3],
    [4, 5, 6],
    [7, 8, 9]
...
function mostrarCartelGanador() {
  swal({
     title: "GANASTE!",
     text: "Nos ahorraste " + movRestantes + " movimientos!",
     icon: "success",
     buttons: ["Salir", "RePlay!"],
     dangerMode: false,
     })
 .then((confirmacion) => {
     if (confirmacion) {
       swal("Intenta superar tu record personal haciendolo en menos movimientos!", {
       icon: "success",
     });
...
function chequearCantMaxMovimientos(){
  if (movRealizados == CantMaxMovimientos){
     swal({
        title: "Woops! Perdiste! ",
        text: "La cantidad maxima de movimientos es " + CantMaxMovimientos +
              " ,sigue intentando!",
        icon: "error",
        buttons: ["Me rindo!", "Una vez mas!"],
        dangerMode: false,
        })
    .then((confirmacion) => {
        if (confirmacion) {
          swal("VAMOOO LXS PIBXXXXXXS!", {
          icon: "success",
        });
          mezclarPiezas(TimesMixPiezas);
          movRealizados = 0;
          ...
          swal("Es hora de descansar el cerebro!");
      }
    });
  }
}
